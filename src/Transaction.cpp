...
bool Transaction::verify(RSA* publicKey) const {
    ...
    // This clears the OpenSSL error stack, ensuring that any errors from previous operations are not carried over and interfere with the current verification process.
    ERR_clear_error();
   ...
   /// checks if the signature provided matches hash using the public key 
