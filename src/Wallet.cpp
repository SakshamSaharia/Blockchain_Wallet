...
int random_number(){
    ...
    // Distribution: generate random integers in range [1, 100]
...
Transaction Wallet::sendFunds(Wallet& receiver, float amount) {
    // Generate a nonce as a random number ( for simplicity)
    int nonce = random_number();  

    Transaction tx(id, receiver.id, amount, nonce);
    tx.sign(privateKey);  // Sign the transaction
    std::cout << "This is the privateKey of sender: "<<privateKey << std::endl;

    return tx;
}
// Method to update balance based on the blockchain transactions
// called when notify wallets is called after every new block 
