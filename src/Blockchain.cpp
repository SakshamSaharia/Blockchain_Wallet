...
// if a block's hash is valid then it must be equal to the hash generated form generate hash function since nonce remains the same if 
...
// amount transferred in a transaction must be positive ( from sender to reciever)
...
bool Blockchain::isChainValid() {
    for (int i = 1; i < chain.size(); ++i) {
        ...
        for (const auto& tx : currBlock.transactions) {
            ...
            // validating transactions ( which calls verify function with public key as parameter to check if signature matches hashvalue)
